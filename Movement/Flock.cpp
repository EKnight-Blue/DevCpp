...
Flock::Flock(Animal const animal, const float size, const size_t nb_members,
             float w, float h)
    : animal{animal}, size{size}, triangles{sf::Quads, 4 * nb_members},
      members(nb_members), world_width{w}, world_height{h} {
...
Flock::Flock(Animal const animal, const float size, const size_t nb_members,
             float w, float h, Ihm &interface)
    : animal{animal}, size{size}, triangles{sf::Quads, 4 * nb_members},
      members(nb_members), world_width{w}, world_height{h},
      abonnements(NB_SUJETS) {
    texture.loadFromFile("./resources/texture.png");
    ...
    for (size_t row{0}; row < rows; ++row) {
        for (size_t column{0}; column < columns; ++column) {
            members[row * columns + column].position = {
                static_cast<float>(column + 1) * width /
                    static_cast<float>(columns + 1),
                static_cast<float>(row + 1) * height /
...
void Flock::update(sf::Time const delta_time) {
    for (auto &member : members) {
        member.age =
            ...
            (frame_number[accumulated_state_counts[static_cast<size_t>(
                              animal)] +
                          member.state] *
             ANIMATION_FRAME_TIME_MS);
        float force = sqrtf(member.force.x * member.force.x +
                            member.force.y * member.force.y);
        if (force == 0.f) {
            // speed is unchanged
            member.position +=
            ...
            member.position += sf::Vector2f{
                world_width *
                    (static_cast<float>(member.position.x < 0.f) -
                     static_cast<float>(member.position.x > world_width)),
                world_height *
                    (static_cast<float>(member.position.y < 0.f) -
                     static_cast<float>(member.position.y > world_height))};
            continue;
        }

        sf::Vector2f speed;
        if (force > max_force)
            speed = member.speed * member.orientation +
                    member.force * (delta_time.asSeconds() * max_force / force);
        else
            speed = member.speed * member.orientation +
                    member.force * delta_time.asSeconds();
        member.force = {0., 0.};
...
sf::Vector2f Flock::difference(const sf::Vector2f &v1,
                               const sf::Vector2f &v2) const {
    sf::Vector2f v = v1 - v2;
    return v +
           sf::Vector2f{
               world_width * (static_cast<float>(v.x < -world_width * .5f) -
                              static_cast<float>(v.x > world_width * .5f)),
               world_height * (static_cast<float>(v.y < -world_height * .5f) -
                               static_cast<float>(v.y > world_height * .5f))};
}
...
std::array<sf::Vector2f const, static_cast<size_t>(Animal::Count)> const
    Flock::anchor = {{
        // Squirrel
        {0.f, 0.f},
        // Rabbit
...
std::array<uint8_t, static_cast<size_t>(
                        Animal::Count)> const Flock::accumulated_state_counts{
    0,

    static_cast<uint8_t>(Squirrel::Count),

    static_cast<uint8_t>(Squirrel::Count) + static_cast<uint8_t>(Rabbit::Count),

    static_cast<uint8_t>(Squirrel::Count) +
        static_cast<uint8_t>(Rabbit::Count) + static_cast<uint8_t>(Bird::Count),
    ...
    static_cast<uint8_t>(Squirrel::Count) +
        ...
        static_cast<uint8_t>(FemaleDeer::Count) +
        static_cast<uint8_t>(MaleDeer::Count) +
        static_cast<uint8_t>(Wolf::Count) + static_cast<uint8_t>(Bear::Count),
