...
bool QuadTree::arc_line(const sf::Vector2f &v1, sf::Vector2f const &v2, float sq_radius, float cos_fov, const sf::Vector2f &orientation) const {
    float radius{sqrtf(sq_radius)};
    if (v1.x * v1.x < sq_radius) {
        float dy{sqrtf(sq_radius - v1.x * v1.x)};
        float ny{top_left.y - v1.y + dy};
        if (0.f <= -v1.y + dy && ny <= bottom_right.y && dot(orientation, sf::Vector2f{v1.x, dy}) >= cos_fov * radius)
            return true;
        ny = top_left.y - v1.y - dy;
        if (0.f <= -v1.y + dy && ny <= bottom_right.y && dot(orientation, sf::Vector2f{v1.x, -dy}) >= cos_fov * radius)
            return true;
    }
    if (v2.x * v2.x < sq_radius) {
        float dy{sqrtf(sq_radius - v2.x * v2.x)};
        float ny{bottom_right.y - v2.y + dy};
        if (-v2.y + dy <= 0.f && top_left.y <= ny && dot(orientation, sf::Vector2f{v2.x, dy}) >= cos_fov * radius)
            return true;
        ny = bottom_right.y - v2.y + dy;
        if (-v2.y - dy <= 0.f && top_left.y <= ny && dot(orientation, sf::Vector2f{v2.x, -dy}) >= cos_fov * radius)
            return true;
    }
    if (v1.y * v1.y < sq_radius) {
        float dx{sqrtf(sq_radius - v1.y * v1.y)};
        float nx{top_left.x - v1.x + dx};
        if (0.f <= -v1.x + dx && nx <= bottom_right.x && dot(orientation, sf::Vector2f{dx, v1.x}) >= cos_fov * radius)
            return true;
        nx = top_left.x - v1.x + dx;
        if (0.f <= -v1.x - dx && nx <= bottom_right.x && dot(orientation, sf::Vector2f{-dx, v1.x}) >= cos_fov * radius)
            return true;
    }
    if (v2.y * v2.y < sq_radius) {
        float dx{sqrtf(sq_radius - v2.y * v2.y)};
        float nx{bottom_right.x - v2.x + dx};
        if (-v2.y + dx <= 0.f && top_left.y <= nx && dot(orientation, sf::Vector2f{dx, v2.x}) >= cos_fov * radius)
            return true;
        nx = bottom_right.x - v2.x + dx;
        if (-v2.y - dx <= 0.f && top_left.y <= nx && dot(orientation, sf::Vector2f{-dx, v2.x}) >= cos_fov * radius)
            return true;
    }
    return false;
